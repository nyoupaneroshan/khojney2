// src/app/admin/analytics/analytics-content.tsx
'use client';
        ...
        <div className="bg-gray-800/50 border border-gray-700/80 rounded-xl p-6 h-full min-h-[300px]">
            ...
            {questions.length === 0 ? (
                ...
                <ul className="space-y-3 text-sm">
                    {questions.map((q: any) => (
                        <li key={q.question_id} className="flex justify-between items-center border-b border-gray-700/50 pb-2 last:border-b-0">
                            <span className="text-gray-300 w-4/5 truncate" title={q.questions.question_text_en}>{q.questions.question_text_en}</span>
                            <span className={`font-bold ${colorClass}`}>{q.correct_percentage.toFixed(1)}%</span>
                        </li>
                    ))}
                </ul>
            )}
            ...
            {users.length === 0 ? (
                ...
                <ol className="space-y-3 text-sm">
                    {users.map((user, index) => (
                        <li key={user.id} className="flex items-center justify-between border-b border-gray-700/50 pb-2 last:border-b-0">
                            <div className="flex items-center gap-3">
                                <span className="font-bold text-gray-500 w-6 text-center">{index + 1}</span>
                                <span className="text-gray-200 truncate">{user.full_name || 'Unnamed User'}</span>
                            </div>
                            <span className="font-bold text-white">{user.total_score || 0} pts</span>
                        </li>
                    ))}
                </ol>
            )}
    ...
    return (
        <div className="col-span-12 bg-gray-800/50 border border-gray-700/80 rounded-xl p-6">
            <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="font-bold text-white">Quiz Performance Details</h3>
            </div>
            {data.length === 0 ? (
...
export default function AnalyticsContent() {
    ...
    const fetchAnalyticsData = useCallback(async (userId: string, currentRange: DateRange | undefined) => {
        ...
        try {
            ...
            const fetchDataForPeriod = async (range: { from: Date, to: Date }, isMainPeriod: boolean) => {
                ...
                if (isMainPeriod && questionStatsRes.error) console.error(`Failed fetching question_stats view: ${questionStatsRes.error?.message}`);
                if (isMainPeriod && leaderboardRes.error) console.error(`Failed fetching leaderboard data: ${leaderboardRes.error?.message}`);
                if (isMainPeriod && quizPerformanceRes.error) console.error(`Failed fetching RPC: ${quizPerformanceRes.error?.message}`);
            ...
            const currentPeriodNewUsers = mainPeriodData.signups.length;
            ...
            const attemptsByDay = mainPeriodData.attempts.reduce((acc: Record<string, number>, a: any) => { 
            ...
            const userSignupsByDay = mainPeriodData.signups.reduce((acc: Record<string, number>, u: any) => { 
            ...
            const prevPeriodNewUsers = prevPeriodData.signups.length;
            ...
            setData({
                kpis: { 
                    ...
                    totalUsers: currentPeriodNewUsers 
                },
                kpiChanges: {
                    totalQuizzes: calculateChange(totalQuizzes, prevTotalQuizzes),
                    avgScore: calculateChange(avgScore, prevAvgScore),
                    totalUsers: calculateChange(currentPeriodNewUsers, prevPeriodNewUsers),
                },
                categoryChartData,
                activityChartData,
                questionStats: {
                    hardest: (mainPeriodData.questionStats || [])
                        .sort((a, b) => a.correct_percentage - b.correct_percentage)
                        .slice(0, 5),
                    easiest: (mainPeriodData.questionStats || [])
    ...
    // Data Merging Logic for LineChart
        ...
        <main className="min-h-screen w-full p-4 sm:p-8 bg-gray-900 text-white">
            <div className="mx-auto max-w-7xl">
                <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-8">
                    <div>
                        <Link href="/admin" className="inline-flex items-center gap-2 text-sm text-cyan-400 hover:text-cyan-300 mb-2">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 items-center">
                        <Popover>
                            <PopoverTrigger asChild>
                                <Button 
                                    id="date" 
                                    variant={"outline"} 
                                    className="w-full sm:w-[280px] justify-start text-left font-normal bg-gray-800 border border-gray-600 hover:bg-gray-700 hover:text-white"
                                >
                        ...
                        <select 
                            ...
                            className="w-full sm:w-auto bg-gray-800 border border-gray-600 rounded-lg py-2.5 px-3 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500"
                        >
                        ...
                        <Button 
                            ...
                            className="w-full sm:w-auto bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50 flex items-center gap-2"
                        >
                            {isFiltering ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Apply'}
                        </Button>
                        <Button 
                            onClick={handleExport} 
                            disabled={!data || data.rawAttemptsData.length === 0} 
                            variant="outline" 
                            className="w-full sm:w-auto bg-gray-800 border-gray-600 hover:bg-gray-700 disabled:opacity-50 flex items-center gap-2"
                        >
                ...
                {isFiltering ? (
                    <div className="space-y-8">
                        ...
                        <div className="grid grid-cols-12 gap-6">
                            <div className="col-span-12 lg:col-span-7"><SkeletonLoader /></div>
                            <div className="col-span-12 lg:col-span-5"><SkeletonLoader /></div>
                        </div>
                    </div>
                ) : !data || data.kpis.totalQuizzes === 0 ? (
                    <div className="text-center h-96 flex items-center justify-center rounded-lg bg-gray-800/50">
                        <EmptyState message="No quiz attempts found for this selection." icon={<BarChart3 size={32} />} />
                    </div>
                ) : (
                    <motion.div 
                        initial={{ opacity: 0 }} 
                    ...
                    >
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                            <StatCard 
                                ...
                                trend={data.kpiChanges.totalUsers} 
                            />
                            <StatCard 
                                icon={<BarChart3 size={24} />} 
                                title="Total Quizzes Taken" 
                                value={data.kpis.totalQuizzes} 
                                trend={data.kpiChanges.totalQuizzes} 
                            />
                            <StatCard 
                                icon={<Target size={24} />} 
                                title="Average Score" 
                                value={data.kpis.avgScore} 
                                trend={data.kpiChanges.avgScore} 
                                unit=" pts" 
                            />
                            <StatCard 
                                icon={<Star size={24} />} 
                        ...
                        <div className="grid grid-cols-12 gap-6">
                            ...
                            <div className="col-span-12 lg:col-span-5">
                                <LeaderboardWidget users={data.leaderboardData} />
                            </div>

                            <div className="col-span-12">
                                <QuizPerformanceWidget data={data.quizPerformanceData} />
                            </div>

                            <div className="col-span-12 lg:col-span-6">
                                <QuestionStatsWidget 
                                    title="Easiest Questions" 
                                    questions={data.questionStats.easiest} 
                                    icon={<TrendingUp />} 
                            ...
                            <div className="col-span-12 lg:col-span-6">
                                <QuestionStatsWidget 
                                    ...
                                    questions={data.questionStats.hardest} 
                                    icon={<TrendingDown />} 
